...
int jugarRonda(string jugadorActivo, int &dadosJugador, int &dadosContrincante) {
    ...
    cin  >> cantElegir;
    while (cantElegir < 1 || cantElegir > dadosJugador && cantElegir ==0) {
        cout << "Cantidad incorrecta. Contas con "<< dadosJugador <<" dados. volve a elegir. "<< endl;
        cin  >> cantElegir;
    }
    ...
    for (int k = 0; k < cantElegir; ++k) {
        ...
        while (pos < 1 || pos > dadosJugador) {
            cout << "Posicion incorrecta. Contas con " << dadosJugador << "dados. Volve a elegir"<<endl;
            cin >> pos;
        }

        suma +=vDados6Caras[pos-1];
        cout << "El dado vale "<< vDados6Caras[pos-1]<< ", suma: "<< suma<< endl;
