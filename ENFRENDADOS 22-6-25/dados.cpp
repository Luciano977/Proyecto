...
int jugarRonda(string jugadorActivo, int &dadosJugador, int &dadosContrincante) {
    ...
    int cantElegir=0;
    while (true) {
    cout << "¨Cu ntos dados deseas elegir? (1 a " << dadosJugador << "): ";
    cin >> cantElegir;

    if (cin.fail() || cantElegir < 1 || cantElegir > dadosJugador) {
        cin.clear(); /// limpiar error de entrada
        cin.ignore(1000, '\n'); /// limpiar el buffer
        cout << "Entrada inv lida. Solo n£meros enteros entre 1 y " << dadosJugador << ".\n";
    } else {
        break;
    }
}
        int sumaElegida = 0, suma = 0, pos;
    bool usados[11] = {};  // ponemos a todos en false por defecto

    for (int i = 0; i < cantElegir; i++) {
        while (true) {
            cout << " Elige el dado (1 al " << dadosJugador << "): ";
            cin >> pos;

            if (cin.fail() || pos < 1 || pos > dadosJugador) {
                cin.clear();
                cin.ignore(1000, '\n');
                cout << "Posici¢n inv lida. Intenta de nuevo.\n";
                continue; //salta a la siguiente iteracion si el dado esta en uso
            }

            if (usados[pos - 1]) {
                cout << "Dado en uso. Por favor elegir otro."<<endl;
                continue;
            }

            usados[pos - 1] = true;
            sumaElegida += vDados6Caras[pos - 1];
            cout << "El dado vale " << vDados6Caras[pos - 1] << ", suma parcial: " << sumaElegida << endl;
            break; /// salir del while interno si todo da ok
        }
    }
